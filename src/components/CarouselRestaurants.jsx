  ...
  useEffect(() => {
    async function gd() {
      const res = await fetch(
        `https://www.swiggy.com/dapi/restaurants/list/v5?lat=28.65200&lng=77.16630&collection=${id}&tags=layout_CCS_Burger&sortBy=&filters=&type=rcv2&offset=0&page_type=null`
      );
          ...
          data.map((item) => {
            ...
            return (
              <Card
                ...
                id={obj.id}
                img={obj.cloudinaryImageId}
                c={"tr"}
                name={obj.name}
                time={obj.sla.deliveryTime}
